...
export default function ReflexologieAvancee() {
  return (
    <>
      <Hero
        route="avancee"
        ...
        badge={
          <>
            <Pastille>{prixDuSoin('avancee')}</Pastille>
            <span className="text-xs font-light text-white/80">{T.avancee.heroBadge}</span>
          </>
        }
        titre={T.avancee.heroTitre}
        sous={T.avancee.heroSous}
        actions={
          <>
        ...
        <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[1fr_0.72fr] lg:gap-20">
          <div>
            ...
            <Reveal delai={170} className="mt-14">
              <PhotoSoin
                src="/photos/diplome.webp"
                alt="Certificat « Mettre en œuvre des techniques de réflexologie », école Métaréflexologie"
                ratio="aspect-video"
                cadrage="50% 50%"
                legende="Certification enregistrée auprès de France Compétences — code RS5311."
              />
            </Reveal>
          </div>
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            <Reveal delai={100}>
              <CarteTarif lignes={tarifsDuSoin('avancee')} note={NOTE_TARIF} />
            </Reveal>
          </div>
        </div>
      </section>
      ...
      <AppelFinal
        fond="epi"
        titre={T.avancee.ctaTitre}
        texte={`${T.avancee.ctaTexte} ${SITE.telephone}.`}
      />
    </>
  );
}
