...
import { ADRESSE, PHOTOS, SITE, URLS, VIDEOS } from '../lib/site';
  ...
  return (
    <>
      <Hero
        ...
        titre="Me contacter"
        sous={`Pour prendre rendez-vous ou poser une question, contactez-moi au ${SITE.telephone} ou via le formulaire.`}
        actions={
          <>
        ...
        <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[0.95fr_1fr] lg:gap-20">
          ...
          <div>
            ...
            <Reveal delai={140}>
              ...
              {/* Sa piece s'il nous l'a montree ; sinon un fond video. On ne
                  met pas le cabinet d'un autre praticien sous son nom. */}
              <figure className="mt-10 overflow-hidden rounded-[1.5rem]">
                {PHOTOS.cabinet ? (
                  <>
                    <img
                      src={PHOTOS.cabinet}
                      alt={`Le cabinet de ${SITE.praticien}, ${ADRESSE}`}
                      width={1024}
                      height={768}
                      loading="lazy"
                      decoding="async"
