...
/* Le sous-titre du hero et la pastille annoncaient « CB, especes ou cheque ·
   bons cadeaux » et « Possibilite de commander des bons cadeaux par courrier,
   mail ou telephone » en dur. C'etaient les conditions de Brigitte, promises
   sur chaque site copie. On n'annonce plus que ce que le praticien a declare —
   et s'il n'a rien declare, le sous-titre retombe sur ses prestations. */
const SOUS_TITRE =
  [TARIFS.note, TARIFS.paiement, TARIFS.bonsCadeaux].filter(Boolean).join(' ') ||
  `Les tarifs de ${SITE.marque}, à ${SITE.adresse.ville}.`;
    ...
    <>
        ...
        badge={
          <>
            ...
            <span className="text-xs font-light text-white/80">
              {SITE.metierCourt} · {SITE.adresse.ville}
            </span>
          </>
        }
        titre="Mes tarifs"
      ...
      >
        <div className="mx-auto max-w-4xl">
          ...
          {/* Les deux cartes sont optionnelles. La plupart des praticiens ne
              vendent pas de bons cadeaux : la carte s'affichait quand meme,
              avec son cadeau dore et rien dessous. */}
          {(TARIFS.paiement || TARIFS.bonsCadeaux) && (
