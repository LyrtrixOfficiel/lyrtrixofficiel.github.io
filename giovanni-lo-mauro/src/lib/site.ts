...
/**
 * Les liens entre pages, **relatifs**.
 *
 * Ils etaient absolus (`/soin-1.html`). Ca marche tant qu'un site est seul a
 * la racine de son domaine — mais les maquettes de prospection sont publiees
 * cote a cote sous un sous-dossier chacune, et `/soin-1.html` renvoyait alors
 * a la racine commune : toute la navigation cassait des la deuxieme page.
 *
 * En relatif, `./soin-1.html` depuis /carole-mast/ mene a
 * /carole-mast/soin-1.html, et le meme code marche a la racine d'un vrai
 * domaine le jour ou la cliente achete. `base: './'` dans vite.config faisait
 * deja ce travail pour les assets ; il manquait pour les liens.
 */
...
/**
 * Fonds video. Chaque entree a son poster WebP, affiche avant que la video
 * charge.
 *
 * Chemins **relatifs**, comme URLS et pour la meme raison : en absolu
 * (`/video/onde.mp4`), une maquette publiee sous /carole-mast/ allait chercher
 * ses videos a la racine du domaine. Le poster s'affichait, la video ne
 * chargeait jamais, et rien dans la console ne le disait clairement.
 */
export const VIDEOS = {
